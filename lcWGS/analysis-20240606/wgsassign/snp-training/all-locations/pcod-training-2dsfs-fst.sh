...
#SBATCH --output=/home/lspencer/pcod-lcwgs-2023/analysis-20240606/wgsassign2/err-out/2dsfs-fst-pops_%A_%a.out
...
saf_dir=${base}/pop-saf
sfs_dir=${base}/pop-2dsfs
fst_dir=${base}/pop-fst
...
#awk 'NR==FNR {a[NR]=$1; n=NR; next} {for (i=1; i<=n; i++) if (a[i] < $1) print a[i], $1}' OFS='\t' ${base}/populations.txt ${base}/populations.txt | sort -k1,1 > ${base}/pops-combos.txt

# Get the specific line for this array job
region1=$(awk -v line=$SLURM_ARRAY_TASK_ID 'NR==line {print $1}' ${base}/pops-combos.txt)
region2=$(awk -v line=$SLURM_ARRAY_TASK_ID 'NR==line {print $2}' ${base}/pops-combos.txt)
