...
Originally run using the G.macrocephalus coding sequences (CDS) genome from NCBI (stored at https://gannet.fish.washington.edu/seashell/snaps/Gadus_macrocephalus.coding.gene.V1.cds), but I want to rerun this using the NCBI G.macrocephalus *transcriptome*
```{r download-query, engine='bash'}
curl https://owl.fish.washington.edu/halfshell/genomic-databank/GCF_031168955.1_ASM3116895v1_rna.fna\
-k \
> ../data/GCF_031168955.1_ASM3116895v1_rna.fna
```

Exploring what fasta file

```{r view-query, engine='bash', eval=TRUE}
head -3 ../data/GCF_031168955.1_ASM3116895v1_rna.fna
```

```{r view2-query, engine='bash', eval=TRUE}
echo "How many sequences are there?"
grep -c ">" ../data/GCF_031168955.1_ASM3116895v1_rna.fna
```

```{r histogram, eval=TRUE}
# Read FASTA file
fasta_file <- "../data/GCF_031168955.1_ASM3116895v1_rna.fna"  # Replace with the name of your FASTA file
...
-query ../data/GCF_031168955.1_ASM3116895v1_rna.fna \
-db ../blastdb/uniprot_sprot_r2023_04 \
-out ../output/03-transcriptome-annotation/Gm.rna-uniprot_blastx.tab \
-evalue 1E-20 \
-num_threads 20 \
-max_target_seqs 1 \
-outfmt 6
...
head -2 ../output/03-transcriptome-annotation/Gm.rna-uniprot_blastx.tab
```

```{r blast-look2, engine='bash', eval=TRUE}
echo "Number of lines in output"
wc -l ../output/03-transcriptome-annotation/Gm.rna-uniprot_blastx.tab
```
...
tr '|' '\t' < ../output/03-transcriptome-annotation/Gm.rna-uniprot_blastx.tab \
> ../output/03-transcriptome-annotation/Gm.rna-uniprot_blastx_sep.tab

head -1 ../output/03-transcriptome-annotation/Gm.rna-uniprot_blastx_sep.tab
...
bltabl <- read.csv("../output/03-transcriptome-annotation/Gm.rna-uniprot_blastx_sep.tab", sep = '\t', header = FALSE)
...
write.table(annot_tab, file = "../output/03-transcriptome-annotation/G_macrocephalus_rna_IDmapping_2024_04_17.tab", sep = "\t",
            row.names = TRUE, col.names = NA)
```

```{bash}
head -n 3 ../output/03-transcriptome-annotation/G_macrocephalus_rna_IDmapping_2024_04_17.tab
```
