...
We can get a higher-level summary of DEG function using Uniprot keywords, instead of the more specific GO terms. We need to retrieve these from the [NIH DAVID bioinformatics tool](https://david.ncifcrf.gov/).

First, isolate lists of Uniprot Acession number from each set off significant DEGs.
```{r get-uniprot-acession-nums}
...
```{r format-acession-nums, engine='bash'}
...
Now we can download these formatted lists of acession numbrs and run them through DAVID to obtain lists of associated Uniprot keywords. Unfortunately I don't think this can be done from command line though, so I'll be using the online tool. DAVID doesn't have *Gadus macrocephalus* as a background option, so I used the closely related *Gadus morhua* (Atlantic cod) instead.

```{r load-DAVID-files, engine='bash'}
curl https://david.ncifcrf.gov/data/download/tr_F2178592E6FB1714596214612.txt > ../output/08-cod-RNAseq-GO-annotation/Gmac_DAVID_L.9.0.tab

curl https://david.ncifcrf.gov/data/download/tr_F2178592E6FB1714596785683.txt > ../output/08-cod-RNAseq-GO-annotation/Gmac_DAVID_L.9.5.tab

curl https://david.ncifcrf.gov/data/download/tr_F2178592E6FB1714596890158.txt > ../output/08-cod-RNAseq-GO-annotation/Gmac_DAVID_L.9.16.tab
```

```{r load-David-files-in-R}
DEGs_DAVID_L.9.0 <- read.delim("../output/08-cod-RNAseq-GO-annotation/Gmac_DAVID_L.9.0.tab", sep="\t")
DEGs_DAVID_L.9.5 <- read.delim("../output/08-cod-RNAseq-GO-annotation/Gmac_DAVID_L.9.5.tab", sep="\t")
DEGs_DAVID_L.9.16 <- read.delim("../output/08-cod-RNAseq-GO-annotation/Gmac_DAVID_L.9.16.tab", sep="\t")
```

Now we can make datasets that include differential expression stats (e.g., `padj`) AND high-level Uniprot keywords.
```{r join-DAVID-and-DEG}
DEGs_DAVID_GO_L.9.0 <- left_join(DEGs_GO_L.9.0_unique, DEGs_DAVID_L.9.0, by=c("V3" = "ID"))
DEGs_DAVID_GO_L.9.5 <- left_join(DEGs_GO_L.9.5_unique, DEGs_DAVID_L.9.5, by=c("V3" = "ID"))
DEGs_DAVID_GO_L.9.16 <- left_join(DEGs_GO_L.9.16_unique, DEGs_DAVID_L.9.16, by=c("V3" = "ID"))
```
...
```{r}
top_25_DEGs_L.9.0 <- head(DEGs_DAVID_GO_L.9.0[order(DEGs_DAVID_GO_L.9.0$padj), ], 25)
...
top_25_DEGs_L.9.5 <- head(DEGs_DAVID_GO_L.9.5[order(DEGs_DAVID_GO_L.9.5$padj), ], 25)
...
top_25_DEGs_L.9.16 <- head(DEGs_DAVID_GO_L.9.16[order(DEGs_DAVID_GO_L.9.16$padj), ], 25)
